...
for kv in kvartal:
    ...
    for i in sp:
        ...
        # Количество последующих зем. участков
        count_zy = 0

        #  Получаем индексы исходного зу из Архивных (исходный эксель)
        index_isxod_arhiv = arhiv.index[arhiv['КН исходного'] == i].tolist()

        # Создаем столбцы в дф, добавляем значения индексов из Архивных (исходный эксель)
        ...
        new_df[f"Последующий Зу_0"] = [arhiv['КН ЗУ'].values[i] for i in index_isxod_arhiv]
        new_df[f"Площадь Зу_0"] = [arhiv['Площадь'].values[i] for i in index_isxod_arhiv]
        new_df[f"Право Зу_0"] = [arhiv['статус'].values[i] for i in index_isxod_arhiv]

        new_df[f"Последующий Зу_1"] = [arhiv['КН образованного'].values[i] for i in index_isxod_arhiv]
        new_df[f"Площадь Зу_1"] = pd.Series([arhiv['Площадь образованного'].values[i] for i in index_isxod_arhiv])
        new_df[f"Право Зу_1"] = pd.Series([arhiv['статус образованного'].values[i] for i in index_isxod_arhiv])


        # Удаляем дубликаты
        new_df = new_df.drop_duplicates()

        # Сбрасываем индексы (после удаления они стоят не по порядку)
